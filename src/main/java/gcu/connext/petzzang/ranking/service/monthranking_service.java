...
@Service
public class monthranking_service {
    @Autowired
    public Monthly_ranking_Repository monthly_ranking_repository;
    ...
    public void save(weeklyrequestdto requestDto) {
        ...
        Monthly_ranking_tb savedata=requestDto.tomonthEntity();
        monthly_ranking_repository.save(savedata);

    }

    @Transactional
    public String update(String id, weeklyupdatedto requestDto) {
        Monthly_ranking_tb ranking = monthly_ranking_repository.findById(id).orElseThrow(()-> new IllegalArgumentException("해당 게시글이 없습니다. id="+id));
    ...
    public Integer findById(String id) {
        Optional<Monthly_ranking_tb> entity = monthly_ranking_repository.findById(id);
